"""
This has no C/VaR to it. Purely for verifying TS works in the standard setting.
"""
import torch
    ...
    while True:
        try:
            ...
            if not random_sampling:
                ...
                inner = PosteriorMean(ts_fantasy)

                candidate_x, candidate_x_value = optimize_acqf(acq_function=inner,
                                                               bounds=dim_bounds,
                                                               q=q,
                                                               num_restarts=num_restarts,
                                                               raw_samples=num_restarts * raw_multiplier)
...
if __name__ == "__main__":
    ...
    num_restarts = 100
    ts_k = 500
    out = full_loop('branin', seed=534, iterations=50, ts_k=ts_k, noise_std=0.1,
                    num_restarts=num_restarts, verbose=False, cuda=False,
                    random_sampling=True, fixednoisegp=False)
    print(out)
