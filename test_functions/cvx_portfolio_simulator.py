...
class CVXPortfolioSimulator(SyntheticTestFunction):
    ...
    def __init__(self, dim: int = 5, experiment_id: Optional[str] = '__', noise_std: Optional[float] = None,
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        ...
        for i in range(input_shape[0]):
            for j in range(input_shape[1]):
                ...
                output[i, j, 0] = self._evaluate_with_unscaled_input(gamma_risk, gamma_tcost, gamma_holding,
                                                                     bid_ask_spread, borrow_cost)
        if flag:
    ...
    def _evaluate_with_unscaled_input(self, gamma_risk, gamma_tcost, gamma_holding, bid_ask_spread, borrow_cost):
        ...
        subprocess.call(['bash', self.script_dir + '/make_copy_of_risk_model.sh', self.datadir + 'risk_model.h5',
