...
class CVXPortfolioSimulator(SyntheticTestFunction):
    ...
    def __init__(self, dim: int = 5, experiment_id: Optional[str] = None, noise_std: Optional[float] = None, negate: bool = False) -> None:
        self.dim = dim
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        ...
        for i in range(input_shape[0]):
            for j in range(input_shape[1]):
                ...
                output[i, j, 0] = self._evaluate_with_unscaled_input(gamma_risk, gamma_tcost, gamma_holding, bid_ask_spread, borrow_cost)
    ...
    def _evaluate_with_unscaled_input(self, gamma_risk, gamma_tcost, gamma_holding, bid_ask_spread, borrow_cost):
        ...
        simulated_tcost = cp.TcostModel(half_spread=bid_ask_spread / 2., nonlin_coeff=1., sigma=sigmas, volume=volumes)
        simulated_hcost = cp.HcostModel(borrow_costs=borrow_cost)
        simulator = cp.MarketSimulator(returns, costs=[simulated_tcost, simulated_hcost],
                                       market_volumes=volumes, cash_key='USDOLLAR')
    
        return_estimate = pd.read_csv(self.datadir + 'return_estimate.csv.gz', index_col=0, parse_dates=[0]).dropna()
        volume_estimate = pd.read_csv(self.datadir + 'volume_estimate.csv.gz', index_col=0, parse_dates=[0]).dropna()
        sigma_estimate = pd.read_csv(self.datadir + 'sigma_estimate.csv.gz', index_col=0, parse_dates=[0]).dropna()
    
        optimization_tcost = cp.TcostModel(half_spread=bid_ask_spread / 2., nonlin_coeff=1.,
        ...
        subprocess.call(['bash', self.script_dir + '/make_copy_of_risk_model.sh', self.datadir + 'risk_model.h5', self.datadir + copy_of_risk_model_name])
