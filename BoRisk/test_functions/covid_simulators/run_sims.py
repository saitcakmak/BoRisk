...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python {} yaml-config-file ('fall' or 'base') (optional:simulation-name)".format(
                sys.argv[0]
            )
        )
        exit()
    ...
    if sys.argv[2] == "fall":
        ...
        print(
            "Error: second argument must be 'fall' or 'base', but got {}".format(
                sys.argv[2]
            )
        )
    ...
    if (
        "base_params_to_update" in sim_config
        and sim_config["base_params_to_update"] != None
    ):
        for param, val in sim_config["base_params_to_update"].items():
            if param not in params and param != "contact_trace_testing_frac":
                print(
                    "Configuration attempting to modify non-existent parameter {}".format(
                        param
                    )
                )
                exit()
