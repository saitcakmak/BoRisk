...
class StochasticSimulation:
    def __init__(self, params):
        ...
        self.mild_symptoms_p = np.sum(
            self.severity_prevalence[: self.mild_severity_levels]
        )
        ...
        self.init_S_count = (
            self.pop_size
            - self.init_E_count
            - self.init_pre_ID_count
            - self.init_ID_count
            - self.init_SyID_mild_count
            - self.init_SyID_severe_count
        )
    ...
    def reset_initial_state(self):
        if self.init_ID_prevalence:
            if self.init_ID_prevalence_stochastic:
                init_ID_count = np.random.binomial(
                    self.pop_size, self.init_ID_prevalence
                )
            else:
        ...
        SyID_mild_sample = self.sample_SyID_mild_times(
            self.init_SyID_mild_count + additional_mild
        )
        self.SyID_mild = SyID_mild_sample[1:]

        SyID_severe_sample = self.sample_SyID_severe_times(
            self.init_SyID_severe_count + additional_severe
        )
    ...
    def step_contact_trace(self, new_QI):
        ...
        total_contacts = int(
            resolve_today_QI
            * self.contact_trace_infectious_window
            * self.daily_contacts_lambda
        )
        total_contacts_traced = np.random.binomial(
            total_contacts, self.contact_tracing_c
        )
        total_cases_isolated = np.random.binomial(
            total_contacts_traced, self.exposed_infection_p
        )
        total_contacts_quarantined = min(
            self.S, total_contacts_traced - total_cases_isolated
        )
    ...
    def run_test(self):
        ...
        new_QI = (
            sum(new_QI_from_ID)
            + sum(new_QI_from_SyID_mild)
            + sum(new_QI_from_SyID_severe)
        )
        ...
        if self.pre_ID_state == "detectable":
            ...
            new_QI_from_pre_ID_mild = np.random.binomial(
                new_QI_from_pre_ID, self.mild_symptoms_p
            )
    ...
    def isolate_self_reports(self):
        ...
        severe_self_reports = np.random.binomial(
            self.SyID_severe, self.severe_self_report_p
        )
    ...
    def step(self):
        ...
        free_tot = (
            ...
            + free_susceptible
            + self.R
            + sum(self.E) * self.exposed_infection_p
        )
        ...
        poisson_param = (
            free_infectious * self.daily_contacts_lambda * free_susceptible / free_tot
        )
        ...
        if leave_QI == 0:
            ...
            leave_QI_mild = min(
                np.random.binomial(leave_QI, self.QI_mild / self.QI), self.QI_mild
            )
    ...
    def _append_sim_df(self):
        ...
        if abs(sum(data) - sum(data[-2:]) - self.pop_size) > 0.0001:
            raise (RuntimeError("population has shrunk"))
        if np.sum(data < 0) > 0:
    ...
    def get_current_state_vector(self):
        return np.concatenate(
            [
                ...
                self.SyID_mild,
                self.SyID_severe,
                [self.cumulative_mild],
                [self.cumulative_severe],
            ]
        )
