...
def plot_many_dfs_threshold(
    dfs_dict, threshold=0.1, xlabel="", title="", figsize=(10, 6)
):
...
def run_sensitivity_sims(
    ...
    for val in param_values:
        ...
        perturbed_dfs[val] = run_multiple_trajectories(
            sim_params, ntrajectories=trajectories_per_config, time_horizon=time_horizon
        )
        if verbose:
...
def run_sensitivity_sims_time_dist(
    base_params,
    state_sensitivity,
    param_avg_values,
    param_max_values,
    f=poisson_waiting_function,
    time_horizon=150,
    trajectories_per_config=100,
    verbose=True,
):
    ...
    for index, avg_value in enumerate(param_avg_values):
        ...
        perturbed_dfs[avg_value] = run_multiple_trajectories(
            sim_params, ntrajectories=trajectories_per_config, time_horizon=time_horizon
        )
        if verbose:
            print(
                "Done simulating time in {} with average length {}".format(
                    state_sensitivity, avg_value
                )
            )
    return perturbed_dfs
...
def plot_aip_vs_t(
    ...
    linewidth=10,
    xlabel="Day",
    ylabel="Active Infection Percent",
    title=None,
):
    plt.figure(figsize=figsize)
...
def plot_cip_vs_t(
    ...
    linewidth=10,
    xlabel="Day",
    ylabel="Cumulative Infection Percent",
    title=None,
):
    plt.figure(figsize=figsize)
