...
def subdivide_severity(prob_severity_given_age, prob_infection, prob_age):
    ...
    for severity_level in range(num_severity_levels):
        ...
        for age_group in range(num_age_groups):
            total += (
                prob_severity_given_age[age_group, severity_level]
                * prob_infection[age_group]
                * prob_age[age_group]
            )
        S.append(total)
...
if __name__ == "__main__":
    ...
    prob_severity_given_age = np.array(
        [
            age_group_1_severity_dist,
            age_group_2_severity_dist,
            age_group_3_severity_dist,
            age_group_4_severity_dist,
            age_group_5_severity_dist,
        ]
    )
