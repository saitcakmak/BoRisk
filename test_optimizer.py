...
class Optimizer:
    ...
    def optimize_VaRKG(self, acqf: VaRKG, w_samples: Tensor) -> Tuple[Tensor, Tensor]:
        """
        ...
        solutions = None
        values = None
        w_sols = torch.empty((w_samples.size(0), 1, self.full_dim))
        w_vals = torch.empty((w_samples.size(0), 1))
        # fix the w_samples of the solutions, then pick the maximizer
        for j in range(w_samples.size(0)):
            # this is purely experimental
            options = {'maxiter': min(self.periods, self.maxiter)}
            fixed_features = {self.dim_x: float(w_samples[j])}
            ...
            w_sols[j] = solutions[best].detach()
            w_vals[j] = values[best].detach()
        best = torch.argmax(w_vals)
        return w_sols[best], w_vals[best]
    ...
    def generate_full_raw_samples(self) -> Tensor:
        """
