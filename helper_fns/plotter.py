...
def contour_plotter(model, inner_var=None, best_pt=None, best_val=None, next_pt=None):
    ...
    if inner_var is not None:
        ...
        with torch.no_grad():
            for i in range(100):
                raw_sobol = draw_sobol_normal_samples(d=alternate_inner.num_samples,
                                                      n=alternate_inner.num_fantasies,
                                                      seed=alternate_inner.inner_seed)
                alternate_inner.sobol_samples = raw_sobol.reshape(alternate_inner.num_repetitions,
                                                                  alternate_inner.num_fantasies, 1,
                                                                  alternate_inner.num_samples, 1)
        ...
        ax[2].fill_between(sols.reshape(-1), mean - 1.96 * std, mean + 1.96 * std, alpha=0.3)
