...
from torch.distributions import Uniform, Gamma, Normal
from VaR_EI import VaREI, InnerVaR
...
def EI_test(start_sol: Tensor):
    """
    this is for testing VaREI
    :param start_sol: starting solution (1 x dim) or value to evaluate
    :return: None
    """
    # construct the acquisition function
    var_ei = VaREI(model=gp, distribution=dist, num_samples=100, alpha=0.7,
                   num_fantasies=10, dim_x=1, num_inner_restarts=2, l_bound=0, u_bound=1)
    # optimize it
    candidates, values = gen_candidates_scipy(start_sol, var_ei, 0, 1)
    print(candidates, values)


def inner_test(sols: Tensor, num_samples: int = 100, alpha: Union[Tensor, float] = 0.7, mode: int = 0):
    """
    ...
    :param sols: Points to evaluate VaR() at (num_points x dim_x)
    ...
    """
    # construct the acquisition function
    inner_VaR = InnerVaR(model=gp, distribution=dist, num_samples=num_samples, alpha=alpha, mode=mode)
    # return the negative since inner VaR negates by default
    return -inner_VaR(sols)


def inner_opt_test(sols: Tensor, num_samples: int = 100, alpha: Union[Tensor, float] = 0.7, mode: int = 0):
    """
...
# test calculation of VaR
# k = 3
# start_sols = torch.linspace(0, 1, k).view(-1, 1)
# VaRs = inner_test(start_sols, 10000, 0.7, 0)
# print(VaRs)

# test optimization of VaR
# k = 3
# start_sols = torch.linspace(0, 1, k).view(-1, 1)
# cand, vals = inner_opt_test(start_sols, 10000, 0.7, 0)
